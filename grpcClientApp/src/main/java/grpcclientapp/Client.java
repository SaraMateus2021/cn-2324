...
public class Client {
    ...
    private static String svcIP = "34.88.126.31"; //localhost
    ...
    static void findPrimesFiveCalls() throws InterruptedException {
        int start = 1;
        int end = 100;

        for (int i = 1; i <= 5; i++) { // make five calls [1-100] [101-200] [201-300] [3001-400] [401-500]
            noBlockStub.findPrimes(IntervalNumbers.newBuilder().setStart(start).setEnd(end).build(),new PrimesStream(start,end));
            start = end +1;
            end = end +100;
        }
    }
    ...
    private static int Menu() {
        ...
        do {
            ...
            System.out.println(" 6 - Case server stream: does five calls to the find primes operation: Asynchronous call");
